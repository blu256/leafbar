...
#ifndef _TDECM_LEAFBAR_H
#define _TDECM_LEAFBAR_H
...
class LeafbarSettings;
class LeafbarAppletSelector;

class LeafbarConfig : public TDECModule {
  TQ_OBJECT

  public:
    LeafbarConfig(TQWidget *parent, const char *name, const TQStringList &);

    void load();
    void save();

    static bool kickerAlive();
    static bool leafbarAlive();
  ...
  private:
    ...
    // Applets tab
    LeafbarAppletSelector *m_appletSelector;
    TQPushButton *m_appletAbout, *m_appletConfig;
};

#endif // _TDECM_LEAFBAR_H

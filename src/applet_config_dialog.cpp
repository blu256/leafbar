...
#include "applet_config_dialog.moc"

LeafbarAppletConfigDialog::LeafbarAppletConfigDialog(TQWidget *parent, AppletData applet)
: KDialogBase(parent, "leafbar_applet_config", true,
              ...
              KDialogBase::Ok, true)
{
    ...
    LeafbarAppletConfig *(*makeConfigWidget)(TQWidget *, const TQString&) = \
        (LeafbarAppletConfig *(*)(TQWidget *, const TQString&))sym;

    m_config = makeConfigWidget(this, "leafbarappletrc");
...
LeafbarAppletConfigDialog::~LeafbarAppletConfigDialog()
{
    KLibLoader::self()->unloadLibrary(m_applet.libPath());
}

bool LeafbarAppletConfigDialog::changed()
{
    return m_changed;
}

/// slot
void LeafbarAppletConfigDialog::slotOk()
{
    if (changed())
    {
        m_config->save();
...
/// slot
void LeafbarAppletConfigDialog::slotApply()
{
    m_config->save();
    reconfigure();
...
/// slot
void LeafbarAppletConfigDialog::slotDefault()
{
    m_config->reset();
...
/// slot
void LeafbarAppletConfigDialog::slotChanged()
{
    m_changed = true;
...
/// slot
void LeafbarAppletConfigDialog::reconfigure()
{
    DCOPRef leafbar("leafbar", "LeafbarIface");
    DCOPReply reply = leafbar.call("reloadApplet", (TQString)m_applet.id);
    kdDebug() << "reloading " << m_applet.id << endl;
    if (!reply.isValid())
    {
        KMessageBox::sorry(this,
            i18n("Cannot apply applet settings! Leafbar is probably not running."),
            i18n("Error contacting Leafbar")
        );
    }
}

...
#include "version.h"
#include "applet_db.h"
#include "applet_db.moc"

#include <config.h>

static LeafbarAppletDB *appletDB = nullptr;

LeafbarAppletDB* LeafbarAppletDB::instance() {
    if (!appletDB) appletDB = new LeafbarAppletDB();
    return appletDB;
}

LeafbarAppletDB::LeafbarAppletDB() : TQObject(0, "applet_db")
{
    TDEGlobal::locale()->insertCatalogue("leafbar-applets");

    TQString dataPath = TDEGlobal::dirs()->kde_default("data") + "leafbar/";
...
bool LeafbarAppletDB::loadApplet(AppletData &data)
{
...
#endif
    if (df.readType() != "LeafbarApplet" ||
        df.readEntry("X-TDE-Library").left(15) != "leafbar_applet_")
    {
        kdWarning() << "LeafbarPanel: " << df.fileName()
...
#endif
        id = data.library.mid(15).latin1();

    data.id = id;

    if (m_applets.contains(id))
    {
        kdWarning() << "LeafbarPanel: applet data with id '" << id << "' "
...
#endif
    {
        ...
        else
        {
            data.about = new TDEAboutData(
                ...
                df.readEntry("Version", leafbar::version).utf8(),
                data.comment.utf8(),
...
TQValueList<TQCString> LeafbarAppletDB::applets() {
    return m_applets.keys();
}

bool LeafbarAppletDB::contains(TQString id) {
    return m_applets.contains(id.latin1());
}

AppletData &LeafbarAppletDB::operator[](TQString id) {
    return m_applets[id.latin1()];
}

int LeafbarAppletDB::licenseFromString(TQString str)
{

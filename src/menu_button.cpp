...
#include "menu_button.moc"

static KHelpMenu *helpMenu = nullptr;

LeafbarMenuBtn::LeafbarMenuBtn(LeafbarPanel *panel)
  : KPushButton(panel, "LeafbarMenuBtn")
{
    if (!helpMenu)
    {
    ...
    m_menu = new LeafbarRootMenu(panel);
    ...
    TQToolTip::add(this, i18n("Leafbar Menu"));
    TQWhatsThis::add(this, i18n("The Leafbar Menu provides access to installed "
                                "applications, locations and common actions."));
}

LeafbarMenuBtn::~LeafbarMenuBtn()
{}

TQSize LeafbarMenuBtn::sizeHint() const
{
    TQFontMetrics fm(TDEGlobalSettings::generalFont());
    return TQSize(width(), fm.height() * 2);
}

void LeafbarMenuBtn::mousePressEvent(TQMouseEvent *e)
{
...
void LeafbarMenuBtn::showMenu() {
    LeafbarPanel *panel = static_cast<LeafbarPanel *>(parent());

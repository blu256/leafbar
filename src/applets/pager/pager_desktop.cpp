...
LeafbarPagerDesktop::LeafbarPagerDesktop(LeafbarPager *pager, int desktop,
...
LeafbarPagerDesktop::~LeafbarPagerDesktop()
{
    delete m_miniature;
    m_miniature = nullptr;
}

bool LeafbarPagerDesktop::isCurrent()
{
    return m_pager->currentDesktop() == m_desktopNo;
}

void LeafbarPagerDesktop::paintEvent(TQPaintEvent *pe)
{
    TQPainter p(this);
    TQRect r(rect());
...
void LeafbarPagerDesktop::mouseReleaseEvent(TQMouseEvent *me)
{
    switch (me->button())
    {
        case LeftButton:
...
void LeafbarPagerDesktop::drawContents(TQPainter *p)
{

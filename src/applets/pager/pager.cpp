...
extern "C"
{
    ...
    {
        return new LeafbarPager(parent, cfg);
    }

    TDE_EXPORT TQWidget *config(TQWidget *parent, const TQString& cfg)
    {
        return new LeafbarPagerConfig(parent, cfg);
    }
}

LeafbarPager::LeafbarPager(LeafbarPanel *parent, TDEConfig *cfg)
  : LeafbarApplet(parent, cfg, "LeafbarPager"),
...
LeafbarPager::~LeafbarPager()
{
    ZAP(m_twin);
    m_desktops.clear();
}

void LeafbarPager::reconfigure()
{
...
void LeafbarPager::recreateDesktops()
{
    m_desktops.clear();
    LeafbarPagerDesktop *desktop;
    ...
    do
    {
        ...
        for (int j = 1; j <= viewports.width() * viewports.height(); ++j)
        {
            ...
            desktop = new LeafbarPagerDesktop(this, count, viewport);
...
void LeafbarPager::currentDesktopChanged(int desktop)
{
...
void LeafbarPager::currentDesktopViewportChanged(int desktop, const TQPoint& viewport)
{
...
void LeafbarPager::numberOfDesktopsChanged(int desktops)
{
    TQSize s = m_twin->numberOfViewports(m_twin->currentDesktop());
...
void LeafbarPager::desktopGeometryChanged(int desktop)
{
}

void LeafbarPager::desktopNamesChanged()
{
}

void LeafbarPager::windowChanged(WId window)
{
}

void LeafbarPager::desktopUpdated(int desktop)
{
}

void LeafbarPager::updateCurrentDesktop()
{
...
void LeafbarPager::gotoDesktop(int desktop)
{
    KWin::setCurrentDesktop(desktop);
}

void LeafbarPager::checkComposite()
{
#ifdef COMPOSITE
    ...
    if (XCompositeQueryExtension(dpy, &event_base, &error_base))
    {
        ...
        if (!(major > 0 || minor >= 2))
        {
            kdDebug() << "[LeafbarPager] XComposite doesn't allow NamePixmap requests! - Disabling XComposite support" << endl;
            m_hasComposite = false;
        }
        ...
        {
            kdDebug() << "[LeafbarPager] XRender not available! - Disabling XComposite support" << endl;
            m_hasComposite = false;
        }
        ...
        {
            kdDebug() << "[LeafbarPager] XDamage not available! - Disabling XComposite support" << endl;
            m_hasComposite = false;
        }
    }
    ...
    if (m_hasComposite)
    {
        kdDebug() << "[LeafbarPager] XComposite extension support enabled" << endl;
    }
#endif
}

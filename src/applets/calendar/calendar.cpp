...
// Leafbar
#include "calendar.h" // LeafbarCalendar class
#include "calendar.moc" // Generated meta-object class -- always include this!

extern "C"
{
    // Here we export the applet constructor so that Leafbar can understand it.
    // Notice that the function accepts a LeafbarPanel* and a TDEConfig* argument,
    ...
    // Leafbar applet settings. It is shared among multiple applets, so when
    ...
    {
        return new LeafbarCalendar(parent, cfg);
    }
}

// The applet constructor. You have to inherit from LeafbarApplet. Notice the
// two arguments we saw above - here we pass them on to the constructor of
// LeafbarApplet, along with an optional (but recommended) const char *name.
// LeafbarApplet is defined in the leafbar_applet.h header, which is by default
// installed in your TDE include directory.
LeafbarCalendar::LeafbarCalendar(LeafbarPanel *parent, TDEConfig *cfg)
  : LeafbarApplet(parent, cfg, "LeafbarCalendar")
{
    // LeafbarApplet is essentially a TQFrame
    setFrameStyle(TQFrame::StyledPanel | TQFrame::Sunken);

    // Purely aesthetic
    setFixedHeight(TQFontMetrics(TDEGlobalSettings::generalFont()).height() * 7);

    // LeafbarApplet has no layout by default, here we create one.
    ...
    // from the context menu of the Leafbar menu button. Similarly, you can add
...
LeafbarCalendar::~LeafbarCalendar()
{
    delete m_calendar;
    m_calendar = nullptr;
}

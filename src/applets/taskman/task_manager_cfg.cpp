...
#include "task_manager_cfg.moc"

LeafbarTaskManConfig::LeafbarTaskManConfig(TQWidget *parent, const TQString& cfg)
: LeafbarAppletConfig(parent, cfg)
{
    ...
    TQWhatsThis::add(m_autoSaveGroupers, i18n("If this is checked, Leafbar will automatically store groupers states "
...
LeafbarTaskManConfig::~LeafbarTaskManConfig()
{
}

void LeafbarTaskManConfig::reset()
{
    // Behaviour
...
void LeafbarTaskManConfig::load()
{
    auto cfg = config();
...
void LeafbarTaskManConfig::save()
{
    auto cfg = config();
    cfg->setGroup("TaskMan");

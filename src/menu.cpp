...
LeafbarMenu::LeafbarMenu(LeafbarPanel *panel)
  : TDEPopupMenu(panel, "LeafbarMenu"),
    m_search(nullptr)
{
    connect(this, TQ_SIGNAL(aboutToHide()), TQ_SLOT(hideSearch()));
}

LeafbarMenu::LeafbarMenu(LeafbarPanel *panel, KServiceGroup::Ptr group)
 : LeafbarMenu(panel)
{
    populate(group);
}

LeafbarMenu::~LeafbarMenu() {
    ZAP(m_search)
}

LeafbarPanel *LeafbarMenu::panel() {
    return static_cast<LeafbarPanel *>(parent());
}

int LeafbarMenu::addSubmenu(KServiceGroup *group, int id, int index) {
    if (group->noDisplay() || !group->childCount()) return -1;
    LeafbarMenu *submenu = new LeafbarMenu(panel(), group);
    return insertItem(SmallIconSet(group->icon()),
                      ESCAPE_AMPERSAND(group->caption()),
                      submenu, id, index);
}

int LeafbarMenu::addSubmenu(TDEPopupMenu *menu, TQString icon, TQString name,
                           int id, int index)
{
    TQIconSet iconSet;
    if (!icon.isNull()) {
...
int LeafbarMenu::addItem(KService *service, const TQObject *receiver, const char *member,
                       int id, int index)
{
...
int LeafbarMenu::addItem(TQString icon, TQString name, int id, int index) {
    TQIconSet iconSet;
    if (!icon.isNull()) {
        iconSet = SmallIconSet(icon);
    }
...
int LeafbarMenu::addItem(TQString icon, TQString name,
...
void LeafbarMenu::populate(KServiceGroup::Ptr group) {
...
void LeafbarMenu::launch(int id) {
    id -= 100;
    ...
    if (panel()->launch(service)) {
        ...
        kickerKMenuIface.call("slotServiceStartedByStorageId(TQString,TQString)",
                              "LeafbarMenu", service->desktopEntryPath());
    }
}

void LeafbarMenu::keyPressEvent(TQKeyEvent *e) {
...
void LeafbarMenu::search(const TQString &str) {
...
int LeafbarMenu::firstSearchMatch() {
    if (m_search) {
        for (int i = 0; i < count(); ++i) {
